<template>
    <b-modal id="project-modal" class="modal fade" role="dialog"
        ...
        <form id="editProjectForm" class="form-horizontal">
            ...
            <edit-project-modal-user v-for="user in users" :key="user.id" @toggle="toggleUser" v-bind:task_user="task_user(user)" v-bind:user="user" v-bind:task="task" />
  ...
  export default {
    ...
    methods: {
      toggleUser(user) {
        ...
        const task_user_index = this.changed_project_users.findIndex(changed_task_user => {
        ...
        if (task_user_index !== -1) {
          ...
          this.changed_project_users[task_user_index].user_rate = user.user_rate
          this.changed_project_users[task_user_index].role = user.role
          this.changed_project_users[task_user_index].user_checked = user.user_checked
        } else {
          //create
      ...
      saveProject: function(callback) {
        this.$store.dispatch(
          ...
          {project: this.project, project_users: this.changed_project_users }
        )
      }
    }
  }
