    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <ul id="expenseModalTabs" class="nav nav-tabs" role="tablist">
            <li role="presentation" class="active">
              <a href="#expenseEditTabShow" aria-controls="expenseEditTabShow" role="tab" data-toggle="tab">Edit Expense</a>
            </li>
            <li role="presentation">
              <a href="#expenseTableTab" aria-controls="expenseTableTab" role="tab" data-toggle="tab">Expense History</a>
            </li>
          </ul>
          <div class="tab-content">
            <div role="tabpanel" class="tab-pane active" id="expenseEditTabShow">
              <form id="editExpenseForm" class="form-horizontal">
                <input id="modalExpenseId" type="hidden" name="id" v-model="expense.id" />
                <div class="form-group">
                  <label class="control-label col-sm-4" for="expense-modal-project-id">Project: </label>
                  <div class="col-sm-6">
                    <select id="expense-modal-project-id" class="form-control" name="project_id" v-model="expense.project_id">
                      <option>***** Select Project *****</option>
                      <option value="create">Create New Project</option>
                      <option v-for="project in openprojects()" :value="project.id">
                ...
                <div class="form-group">
                  <label class="control-label col-sm-4" for="expenseTaskSelect">Task: </label>
                  <div class="col-sm-8">
                    <select name="task_id" id="expenseTaskSelect" class="form-control" v-model="expense.task_id">
                      <option value="0">***** Select Task *****</option>
                      <option v-for="task in projecttasks(expense.project_id)" :value="task.id">{{ task.title }} </option>
                    </select>
                  </div>
                </div>
                <div v-if="isAdmin()" class="form-group">
                  <label class="control-label col-sm-4" for="expenseUserSelect">User: </label>
                  <div class="col-sm-8">
                    <select name="user_id" id="expenseUserSelect" class="form-control">
                      <option value="">***** Select User *****</option>
                      <option :selected="isExpenseUser(user.user_id)" v-for="user in users" v-bind:value="user.user_id">
                        {{ user.name }}
                      </option>
                      <option>Selected or Current User if Editing Expense?</option>
                    </select>
                  </div>
                </div>
                <div v-if="isAdmin()" class="form-group">
                  <label class="control-label col-sm-4" for="expenseUserTime">Is billable? </label>
                  <div class="col-sm-8">
                    <div class="checkbox">
                      <label>
                        <input name="is_billable" type="checkbox" id="blankCheckbox" value="1" v-model="expense.is_billable" aria-label="..." />
                      </label>
                    </div>
                  </div>
                </div>
                <div v-if="isAdmin()" class="form-group">
                  <label class="control-label col-sm-4" for="report_at">Paid Amount: </label>
                  <div class="col-sm-8">
                ...
                <div v-if="isAdmin()" class="form-group">
                  <label class="control-label col-sm-4" for="client_rate">Invoice Amount: </label>
                  <div class="col-sm-8">
                ...
                <div class="form-group">
                  <label class="control-label col-sm-4" for="expenseUserNotes">Notes: </label>
                  <div class="col-sm-8">
                    <div contenteditable="true" id="expenseUserNotes" class="form-control" style="height: auto; min-height: 35px;" v-html="checkNotes(expense.notes)"></div>
                  </div>
                </div>
                <div v-if="isAdmin()" class="form-group">
                  <label class="control-label col-sm-4" for="expenseInvoiceNotes">Invoice Notes: </label>
                  <div class="col-sm-8">
                    <div contenteditable="true" id="expenseInvoiceNotes" class="form-control" style="height: auto; min-height: 35px;" v-html="checkNotes(expense.invoice_notes)"></div>
                  </div>
                </div>
                <div v-if="isAdmin()" class="form-group">
                  <label class="control-label col-sm-4" for="expenseInvoiceNotes">Admin Notes: (visible to users, not clients)</label>
                  <div class="col-sm-8">
                    <div contenteditable="true" id="expenseAdminNotes" class="form-control" style="height: auto; min-height: 35px;" v-html="checkNotes(expense.admin_notes)"></div>
                  </div>
                </div>
                <div v-if="!isIHI()" class="form-group">
                  <label class="control-label col-sm-4" for="report_at">Started at: </label>
                  <div class="col-sm-8">
                    <input name="report_at" type="datetime" id="report_at" class="form-control" :value="expense.report_at" />
                  </div>
                </div>
              </form>
            </div>
            <div role="tabpanel" class="tab-pane" id="expenseTableTab">
              <table class="table expense-table">
                <tbody>
                  ...
                  <tr v-bind:history="history" v-for="history in expense.histories" :key="history" is="timelog-history-row"></tr>
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div class="modal-footer">
          <button v-if="isCurrentUserOrAdmin()" type="button" @click="deleteExpense()" class="btn btn-danger pull-left">
            Delete
          </button>
          <button type="button" class="btn btn-default" data-dismiss="modal">
            Close
          </button>
          <button type="button" class="btn btn-primary" v-on:click="saveExpense()">
            Save changes
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    client_rate_value: function() {
      return this.expense.default_client_rate != this.expense.client_rate ? this.expense.client_rate : ''
    },
    user_rate_placeholder: function() {
      return this.expense.default_user_rate
    },
    user_rate_value: function() {
      return this.expense.default_user_rate != this.expense.user_rate ? this.expense.user_rate : ''
    },
    isBillable: function() {
    ...
    saveExpense: function(callback) {
      ...
      formData = formData + '&notes=' + encodeURIComponent($('#expenseUserNotes').html()) + '&materials=' + encodeURIComponent($('#expenseUserMaterials').html()) + '&duration=' + self.expense.duration
