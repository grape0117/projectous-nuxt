...
export const actions: ActionTree<IModuleState, IRootState> = {
  async restartTimer(context, timer) {
    // @ts-ignore
    const response = await this._vm.$http().post('/timer/restart/' + timer.id, timer)
    ...
    response.timers.forEach(function(timerFromResponse: any) {
      context.commit('UPSERT', { module: 'timers', entity: timerFromResponse }, { root: true })
    })
  },
  async stopTimer(context, timer) {
    // @ts-ignore
    const response = await this._vm.$http().post('/timer/stop/' + timer.id, timer)
    context.commit('UPDATE', { module: 'timers', entity: response.timer }, { root: true })
  },
  async pauseTimer(context, timer) {
    // @ts-ignore
    const response = await this._vm.$http().post('/timer/pause/' + timer.id, timer)
    context.commit('UPDATE', { module: 'timers', entity: response.timer }, { root: true })
  },
  async hideTimer(context, timer) {
    // @ts-ignore
    const response = await this._vm.$http().post('/timer/hide/' + timer.id, timer)
    context.commit('UPDATE', { module: 'timers', entity: response.timer }, { root: true })
  },
  ...
  async startTimer(context, timer) {
    // @ts-ignore
    const response = await this._vm.$http().post('/timer/start-project-timer', timer)
    ...
    response.timers.forEach(function(t: any) {
      context.commit('UPSERT', { module: 'timers', entity: t }, { root: true })
    })
  },
