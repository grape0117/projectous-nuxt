...
const actions = {
  saveTask: function(context, task) {
    ajax('/task/save/ajax', task, function(response) {
      ...
      context.dispatch(
        'settings/closeModal',
        { modal: 'task', object: response.task, pop: true, push: false },
        { root: true }
      )
...
const getters = {
  ...
  projectTasks: (state, getters, rootState, rootGetters) => project_id => {
    ...
    return tasks.sort(function(a, b) {
      ...
      a.numeric_priority =
        new Date(a.due_date) < inAWeek ? 2 : a.numeric_priority
      b.numeric_priority =
        new Date(a.due_date) < inAWeek ? 2 : b.numeric_priority
  ...
  userTasks: (state, getters, rootState, rootGetters) => user_id => {
    ...
    let userProjects
    return getters['all_tasks'].filter(task => {
      let userMatch = false
      if (task.project_id) {
...
export const tasks = {
  ...
  actions: {
      ...
      if (index !== -1) {
        ...
        if (context.state.tasks[index].project_id != task.project_id) {
          ...
          if (context.state.tasks[index].project_id) {
            context.commit(
              'moveTasksDown',
              context.state.tasks[index].project_id
            )
            task.order = 1
          }
    ...
    createTask: function(context, task) {
      ...
      ajax('task/add', task, function(response) {
        ...
        if (response.project && response.project.id) {
          context.commit('projects/updateProject', response.project, {
            root: true
          })
        }
    ...
    setTaskAttribute: function(context, { attribute, value, task_id }) {
      ...
      ajax('task/' + attribute + '/ajax/' + task_id, { value: value }, function(
        response
      ) {
        alert('update')
    ...
    createTaskFromEnter({ context, task }) {
      console.log(task)
      ajax('/task/createFromEnter', task, function(response) {
        context.state.tasks.push(response.task)
        context.commit('updateLookup')
        //TODO: move to create Task
        context.state.tasks[context.state.lookup[task.from_id]].next_task_id =
          response.task.id
    ...
    saveTaskTitle: function(context, { id, title }) {
      ajax('/task/saveTaskTitle', { id: id, title: title }, function(response) {
