...
export const getters: GetterTree<IModuleState, IRootState> = {
  getById: (state: IModuleState) => (id: any) => {
    return state.shared_project_users[state.lookup[id]]
  },
  // @ts-ignore
  getByProjectIdAndUserId: (state: IModuleState) => (project_id: string, company_user_id: string) => {
    if (!state.shared_project_users.length) {
      return []
    }
    // @ts-ignore
    const found_project_user = state.shared_project_users.find(project_user => project_user.project_id === project_id && project_user.company_user_id === company_user_id)
  ...
  getByProjectIdAndCompanyUserId: (state: IModuleState) => ({ project_id, company_user_id }: any) => {
    ...
    // @ts-ignore
    const found_project_user = state.shared_project_users.find((project_user: any) => {
